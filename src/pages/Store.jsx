...
// User-in hansı storage-də olduğunu tapır
const getUserStorage = (userId) => {
  try {
    const localUser = localStorage.getItem('user');
    if (localUser) {
      const parsedUser = JSON.parse(localUser);
      if (parsedUser.id === userId) return localStorage;
    }
  } catch {}
  ...
  try {
    ...
    if (sessionUser) {
      ...
      if (parsedUser.id === userId) return sessionStorage;
    }
  } catch {}
...
// User-ə uyğun wishlist hook-u
const useUserWishlist = (user) => {
  ...
  useEffect(() => {
    ...
    const stored = storage.getItem(`wishlist_${user.id}`);
  ...
  const updateWishlist = (newWishlist) => {
    ...
    const storage = getUserStorage(user.id);
    if (!storage) return;
    storage.setItem(`wishlist_${user.id}`, JSON.stringify(newWishlist));
...
function Store() {
  ...
  const [pricevalue, setPriceValue] = useState([0, 530]);
  ...
  const [wishlist, setWishlist] = useUserWishlist(user);
  ...
  // fullData set
  useEffect(() => {
    setFullData([...games, ...dlc]);
  }, [games, dlc]);

  // Search params-a görə seçilmiş tiplər
  useEffect(() => {
    if (searchParams.get('type') === 'Games') {
      setSelectedTypes((prev) => ({ ...prev, type: ['games'] }));
    } else if (searchParams.get('type') === 'DLC') {
      setSelectedTypes((prev) => ({ ...prev, type: ['dlcs'] }));
    } else {
  ...
  useEffect(() => {
    ...
    if (selectedTypes.type.length > 0) {
      filtered = filtered.filter((item) => {
        if (selectedTypes.type.includes('games') && item.type === 'basedgame') return true;
        if (selectedTypes.type.includes('dlcs') && item.type === 'DLC') return true;
        return false;
      });
    }
    ...
    if (selectedTypes.genre.length > 0) {
      filtered = filtered.filter((item) => {
        ...
        return selectedTypes.genre.some((g) => item.genre.includes(g));
      });
    }

    if (selectedTypes.brand.length > 0) {
      filtered = filtered.filter((item) => selectedTypes.brand.includes(item.brand));
    }

    if (selectedTypes.dlcType.length > 0) {
      filtered = filtered.filter((item) => selectedTypes.dlcType.includes(item.typeDlc));
    }
